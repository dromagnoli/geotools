...
public class TimeParser {
    ...
    public static final String[] LENIENT_FORMATS_HOUR =
            new String[] {"yyyyMMdd'T'HH'Z'", "yyyy-MM-dd'T'HH", "yyyyMMdd'T'HH"};
    ...
    private static enum FormatAndPrecision {
        ...
        YEAR("yyyy", Calendar.YEAR, LENIENT_FORMATS_YEAR);
     ...
     *
     * @param maxTimes Maximum number of times to parse, or a non positive number to have no limit
     * @param lenient whether to support alternative forms of ISO8601 (as an instance, YYYYMMdd in
     *     addition to YYYY-MM-dd). See the LENIENT_FORMATS_XXX Constants for a list of values
     */
    public TimeParser(int maxTimes, boolean lenient) {
     ...
     * a syntax defined in the #FormatAndPrecision array to be validated.
     *
    ...
    private Object parseTime(FormatAndPrecision f, String computedValue) {
        ParsePosition pos = new ParsePosition(0);
        Date time = f.getFormat().parse(computedValue, pos);
        Object parsed = parseRangeWithPrecision(time, computedValue, pos, f);
        if (parsed == null && lenient) {
            String[] lenientFormats = f.getLenientFormats();
            for (String lenientFormat : lenientFormats) {
                ...
                // The lenientFormat is already a lenient version of the official format.
                // no need to have it lenient too.
                format.setLenient(false);
                time = format.parse(computedValue, pos);
                parsed = parseRangeWithPrecision(time, computedValue, pos, f);
                if (parsed != null) {
                    return parsed;
                }
            }
    ...
    private Object parseRangeWithPrecision(
            Date time, String computedValue, ParsePosition pos, FormatAndPrecision f) {
        if (pos != null && pos.getIndex() == computedValue.length()) {
