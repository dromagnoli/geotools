...
public final class NetCDFDimensionIndexes {
    ...
    /** Time axis lookup, supporting exact match and lower/upper bound queries. */
    public interface TimeAxisLookup extends AxisLookup {
        int exact(Date value);

        int lowerBound(Date value, boolean inclusive);

        int upperBound(Date value, boolean inclusive);

        Date valueAt(int index);
    }

    /** Numeric axis lookup, supporting exact match and lower/upper bound queries. */
    public interface NumericAxisLookup extends AxisLookup {
        int exact(double value);

        int lowerBound(double value, boolean inclusive);

        int upperBound(double value, boolean inclusive);
    ...
    public static final class RegularTimeAxisLookup implements TimeAxisLookup {
        ...
        @Override
        public int lowerBound(Date value, boolean inclusive) {
            return lowerBoundMillis(value.getTime(), inclusive);
        }

        @Override
        public int upperBound(Date value, boolean inclusive) {
            return upperBoundMillis(value.getTime(), inclusive);
        }
        ...
        private int lowerBoundMillis(long millis, boolean inclusive) {
            if (size == 0) return 0;
            if (step == 0L) {
            ...
            if (step > 0) {
                ...
                int idx = inclusive ? (int) Math.ceil(raw) : (int) Math.floor(raw) + 1;
                return clamp(idx, size);
            } else {
                // descending axis: fallback to generic array semantics without materializing array
                return lowerBoundDescending(millis, inclusive);
            }
        }

        private int upperBoundMillis(long millis, boolean inclusive) {
            if (size == 0) return 0;
            if (step == 0L) {
                if (inclusive ? millis >= start : millis > start) return 1;
                return 0;
            }
            if (step > 0) {
                double raw = (millis - start) / (double) step;
                int idx = inclusive ? (int) Math.floor(raw) + 1 : (int) Math.ceil(raw);
                return clamp(idx, size);
            } else {
                return upperBoundDescending(millis, inclusive);
            }
        }

        private int lowerBoundDescending(long millis, boolean inclusive) {
            int lo = 0, hi = size;
            while (lo < hi) {
                int mid = (lo + hi) >>> 1;
                long v = start + mid * step;
                boolean takeRight = inclusive ? v > millis : v >= millis;
                if (takeRight) lo = mid + 1;
                else hi = mid;
            }
            return lo;
        }

        private int upperBoundDescending(long millis, boolean inclusive) {
            int low = 0, high = size;
            while (low < high) {
                int mid = (low + high) >>> 1;
                long v = start + mid * step;
                boolean takeRight = inclusive ? v >= millis : v > millis;
                if (takeRight) low = mid + 1;
                else high = mid;
            }
            return low;
        }
    ...
    public static final class ArrayTimeAxisLookup implements TimeAxisLookup {
        ...
        @Override
        public int lowerBound(Date value, boolean inclusive) {
            return NetCDFDimensionIndexes.lowerBound(values, value.getTime(), inclusive);
        }

        @Override
        public int upperBound(Date value, boolean inclusive) {
            return NetCDFDimensionIndexes.upperBound(values, value.getTime(), inclusive);
        }
    ...
    public static final class RegularNumericAxisLookup implements NumericAxisLookup {
        ...
        @Override
        public int lowerBound(double value, boolean inclusive) {
            if (size == 0) return 0;
            if (step > 0) {
                double raw = (value - start) / step;
                int idx = inclusive ? (int) Math.ceil(raw - EPS) : (int) Math.floor(raw + EPS) + 1;
                return clamp(idx, size);
            }
            return lowerBoundDescending(value, inclusive);
        }

        @Override
        public int upperBound(double value, boolean inclusive) {
            if (size == 0) return 0;
            if (step > 0) {
                double raw = (value - start) / step;
                int idx = inclusive ? (int) Math.floor(raw + EPS) + 1 : (int) Math.ceil(raw - EPS);
                return clamp(idx, size);
            }
            return upperBoundDescending(value, inclusive);
        }
        ...
        private int lowerBoundDescending(double value, boolean inclusive) {
            int lo = 0, hi = size;
            while (lo < hi) {
                int mid = (lo + hi) >>> 1;
                double v = valueAt(mid);
                boolean takeRight = inclusive ? v > value : v >= value;
                if (takeRight) lo = mid + 1;
                else hi = mid;
            }
            return lo;
        ...
        private int upperBoundDescending(double value, boolean inclusive) {
            int lo = 0, hi = size;
            while (lo < hi) {
                int mid = (lo + hi) >>> 1;
                double v = valueAt(mid);
    ...
    public static final class ArrayNumericAxisLookup implements NumericAxisLookup {
        ...
        @Override
        public int lowerBound(double value, boolean inclusive) {
            return NetCDFDimensionIndexes.lowerBound(values, value, inclusive);
        }

        @Override
        public int upperBound(double value, boolean inclusive) {
            return NetCDFDimensionIndexes.upperBound(values, value, inclusive);
        }
    ...
    public static int lowerBound(long[] values, long target, boolean inclusive) {
    ...
    public static int upperBound(long[] values, long target, boolean inclusive) {
        int lo = 0, hi = values.length;
        while (lo < hi) {
            int mid = (lo + hi) >>> 1;
            if (inclusive ? values[mid] <= target : values[mid] < target) lo = mid + 1;
    ...
    public static int lowerBound(double[] values, double target, boolean inclusive) {
    ...
    public static int upperBound(double[] values, double target, boolean inclusive) {
        final double eps = 1E-9;
        int lo = 0, hi = values.length;
        while (lo < hi) {
            int mid = (lo + hi) >>> 1;
